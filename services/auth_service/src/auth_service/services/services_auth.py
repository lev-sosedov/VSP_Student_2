...
from auth_service.core.core_security import (hash_password, verify_password, create_access_token, create_refresh_token,
                                             decode_token)
from auth_service.schemas.schemas_auth import (RegisterRequest, LoginRequest, RefreshRequest)
...
class AuthService:
    ...
    async def register(self, data: RegisterRequest):
        ...
        user = await self.repo.create_user(
            {"phone_number": data.phone_number, "user_name": data.user_name, "hashed_password": password_hash})
        # RabbitMQ -> user-service
        await publish_user_created({"auth_id": user.id, "phone_number": user.phone_number, "user_name": user.user_name})

        return {"id": user.id, "phone_number": user.phone_number, "message": "User created"}

    async def login(self, data: LoginRequest):
        user = await self.repo.get_user_by_phone(data.phone_number)
