...
async def test_parent_sync_deduplicates_teacher_across_groups(monkeypatch):
    ...
    async def call(*, method, payload, **kwargs):
        ...
        if method == "group_members.get_by_group":
            return {"success": True, "members": [{"user_id": 42}, {"user_id": 1002}]}
        if method == "user.get_by_id":
            user_id = payload["user_id"]
            users = {
                42: {"id": 42, "role": "teacher", "is_active": True},
                99: {"id": 99, "role": "admin", "is_active": True},
            }
            return {"success": True, "user": users.get(user_id)}
...
@pytest.mark.asyncio
async def test_parent_sync_skips_inactive_and_non_teacher_candidates(monkeypatch):
    _FakeService.instances.clear()
    monkeypatch.setattr(api_chat, "ChatService", _FakeService)
    monkeypatch.setattr(api_chat.settings, "ADMIN_USER_ID", 99)
    ...
    async def call(*, method, payload, **kwargs):
        if method == "parent.authorization.students":
            return {"success": True, "student_ids": [7]}
        if method == "academic.authorization.user_groups":
            return {"success": True, "group_ids": [10]}
        if method == "group_members.get_by_group":
            return {"success": True, "members": [{"user_id": 42}, {"user_id": 43}, {"user_id": 44}]}
        if method == "user.get_by_id":
            users = {
                42: {"id": 42, "role": "teacher", "is_active": True},
            ...
            return {"success": True, "user": users.get(payload["user_id"])}
        raise AssertionError(method)
    ...
    await api_chat._sync_parent_private_chats(5, object())
    calls = _FakeService.instances[-1].calls
    assert [entry[1].get("second_user_id") for entry in calls if entry[0] == "teacher"] == [42]
...
@pytest.mark.asyncio
async def test_parent_sync_user_rpc_timeout_fails_before_writes(monkeypatch):
    _FakeService.instances.clear()
    monkeypatch.setattr(api_chat, "ChatService", _FakeService)
    monkeypatch.setattr(api_chat.settings, "ADMIN_USER_ID", 99)
    ...
    async def call_user(*, method, payload, **kwargs):
        if method == "parent.authorization.students":
            return {"success": True, "student_ids": [7]}
        raise TimeoutError("user rpc timeout")

    async def call_academic(*, method, payload, **kwargs):
        if method == "academic.authorization.user_groups":
            return {"success": True, "group_ids": [10]}
        return {"success": True, "members": [{"user_id": 42}]}

    monkeypatch.setattr(api_chat.communication_rpc_client, "call_user", call_user)
    monkeypatch.setattr(api_chat.communication_rpc_client, "call_academic", call_academic)
    with pytest.raises(HTTPException) as error:
        await api_chat._sync_parent_private_chats(5, object())
    assert error.value.status_code == 503
    assert _FakeService.instances == []
