...
def get_group_member_service(session: AsyncSession = Depends(get_session)):
    return GroupMemberService(repo=GroupMemberRepository(session), group_repo=GroupRepository(session),
                              user_client=UserRpcClient(rabbit_rpc_client))


def get_module_service(session: AsyncSession = Depends(get_session)):
    return ModuleService(repo=ModuleRepository(session))


def get_education_plan_service(session: AsyncSession = Depends(get_session)):
...
def get_education_plan_module_service(session: AsyncSession = Depends(get_session)):
...
def get_direction_service(session: AsyncSession = Depends(get_session)):
    return DirectionService(repo=DirectionRepository(session), plan_repo=EducationPlanRepository(session))


def get_branch_service(session: AsyncSession = Depends(get_session)):
    return BranchService(branch_repo=BranchRepository(session), address_repo=BranchAddressRepository(session))


def get_branch_address_service(session: AsyncSession = Depends(get_session)):
    return BranchAddressService(BranchAddressRepository(session))
