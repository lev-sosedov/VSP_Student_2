...
class ModuleRepository:
    ...
    async def get_all(self):
        result = await self.db.execute(select(Module).order_by(Module.order_number))
    ...
    async def search_by_name(self,name: str):
        result = await self.db.execute(select(Module).where(Module.name.ilike(f"%{name}%")).order_by(Module.order_number))
    ...
    async def update(self,module_id: int,data: dict):
        await self.db.execute(update(Module).where(Module.id == module_id).values(**data))
        await self.db.commit()
